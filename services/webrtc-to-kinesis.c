...
GstFlowReturn on_new_sample(GstElement* sink, gpointer data, UINT64 trackid)
{
    ...
    if (!isDroppable) {
        ...
        if (!GST_CLOCK_TIME_IS_VALID(buf_pts)) {
            DLOGE("[WebRTC-Kinesis] Frame contains invalid PTS dropping the frame");
        }

        if (!(gst_buffer_map(buffer, &info, GST_MAP_READ))) {
            DLOGE("[WebRTC-Kinesis] on_new_sample(): Gst buffer mapping failed");
            goto CleanUp;
        }
        ...
        for (i = 0; i < pSampleConfiguration->streamingSessionCount; ++i) {
            ...
            if (status != STATUS_SRTP_NOT_READY_YET && status != STATUS_SUCCESS) {
#ifdef VERBOSE
                DLOGE("[WebRTC-Kinesis] writeFrame() failed with 0x%08x", status);
#endif
                ...
                DLOGI("[WebRTC-Kinesis] SRTP not ready yet, dropping frame");
            }
        }
...
PVOID sendGstreamerAudioVideo(PVOID args)
{
    ...
    CHK_ERR(pSampleConfiguration != NULL, STATUS_NULL_ARG, "[WebRTC-Kinesis] Streaming session is NULL");
                ...
                case RTSP_SOURCE: {
                    UINT16 stringOutcome =
                        SNPRINTF(rtspPipeLineBuffer, RTSP_PIPELINE_MAX_CHAR_COUNT,
                                 "uridecodebin uri=%s ! nvvidconv ! videoconvert !"
                                 "x264enc name=sampleVideoEncoder bframes=0 speed-preset=veryfast bitrate=512 byte-stream=TRUE tune=zerolatency ! "
                    ...
                    if (stringOutcome > RTSP_PIPELINE_MAX_CHAR_COUNT) {
                        DLOGE("[WebRTC-Kinesis] ERROR: rtsp uri entered exceeds maximum allowed length set by RTSP_PIPELINE_MAX_CHAR_COUNT");
                        goto CleanUp;
                    }
        ...
        case SAMPLE_STREAMING_AUDIO_VIDEO:
            switch (pSampleConfiguration->srcType) {
                ...
                case RTSP_SOURCE: {
                    UINT16 stringOutcome =
                        SNPRINTF(rtspPipeLineBuffer, RTSP_PIPELINE_MAX_CHAR_COUNT,
                                 "uridecodebin uri=%s name=src ! nvvidconv ! videoconvert ! "
                                 "x264enc name=sampleVideoEncoder bframes=0 speed-preset=veryfast bitrate=512 byte-stream=TRUE tune=zerolatency ! "
                                 "video/x-h264,stream-format=byte-stream,alignment=au,profile=baseline ! queue ! "
                                 "appsink sync=TRUE emit-signals=TRUE name=appsink-video "
                                 "src. ! audioconvert ! "
    ...
    CHK_ERR(senderPipeline != NULL, STATUS_NULL_ARG, "[WebRTC-Kinesis] Pipeline is NULL");
    ...
    if (!(appsinkVideo != NULL || appsinkAudio != NULL)) {
        DLOGE("[WebRTC-Kinesis] sendGstreamerAudioVideo(): cant find appsink, operation returned status code: 0x%08x", STATUS_INTERNAL_ERROR);
        goto CleanUp;
    }
...
CleanUp:
    ...
    if (error != NULL) {
        DLOGE("[WebRTC-Kinesis] %s", error->message);
        g_clear_error(&error);
    }
...
INT32 main(INT32 argc, CHAR* argv[])
{
    ...
    pSampleConfiguration->srcType = RTSP_SOURCE; // Default to RTSP source

    /* Initialize GStreamer */
    gst_init(&argc, &argv);
    DLOGI("[WebRTC-Kinesis] Finished initializing GStreamer and handlers");
    DLOGI("[WebRTC-Kinesis] Streaming type set to video-only");

    // Set up RTSP source if specified in arguments
    if (argc > 2 && STRCMP(argv[2], "rtspsrc") == 0) {
        DLOGI("[WebRTC-Kinesis] Using RTSP source in GStreamer");
        if (argc < 4) {
            DLOGI("[WebRTC-Kinesis] No RTSP source URI included. Usage: ./webrtc-to-kinesis <channel name> rtspsrc rtsp://<rtsp uri>");
            pSampleConfiguration->srcType = DEVICE_SOURCE;
        } else {
            pSampleConfiguration->srcType = RTSP_SOURCE;
            pSampleConfiguration->rtspUri = argv[3];
        }
    } else {
        DLOGI("[WebRTC-Kinesis] Using device source in GStreamer");
    }

    // Initalize KVS WebRTC. This must be done before anything else, and must only be done once.
    CHK_STATUS(initKvsWebRtc());
    DLOGI("[WebRTC-Kinesis] KVS WebRTC initialization completed successfully");

    CHK_STATUS(initSignaling(pSampleConfiguration, SAMPLE_MASTER_CLIENT_ID));
    DLOGI("[WebRTC-Kinesis] Channel %s set up done ", pChannelName);

    // Checking for termination
    CHK_STATUS(sessionCleanupWait(pSampleConfiguration));
    DLOGI("[WebRTC-Kinesis] Streaming session terminated");

CleanUp:

    if (retStatus != STATUS_SUCCESS) {
        DLOGE("[WebRTC-Kinesis] Terminated with status code 0x%08x", retStatus);
    }

    DLOGI("[WebRTC-Kinesis] Cleaning up....");
    ...
    if (pSampleConfiguration != NULL) {
        ...
        if (retStatus != STATUS_SUCCESS) {
            DLOGE("[WebRTC-Kinesis] freeSignalingClient(): operation returned status code: 0x%08x", retStatus);
        }

        retStatus = freeSampleConfiguration(&pSampleConfiguration);
        if (retStatus != STATUS_SUCCESS) {
            DLOGE("[WebRTC-Kinesis] freeSampleConfiguration(): operation returned status code: 0x%08x", retStatus);
        }
    }
    DLOGI("[WebRTC-Kinesis] Cleanup done");
